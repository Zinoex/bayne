...
def alpha_beta(class_or_obj):
    types = {
        ...
        SurrogateLinear: alpha_beta_linear,
...
def alpha_beta_sequential(class_or_obj):
    ...
    if isinstance(class_or_obj, nn.Sequential):
        add_alpha_beta_submodules(class_or_obj)
...
def alpha_beta_relu(class_or_obj):
    ...
    def alpha_beta(self: nn.ReLU, layer_bound: LayerBound, adaptive_relu: bool = True, **kwargs) -> AlphaBeta:
        ...
        z = UB / (UB - LB)
        if adaptive_relu:
            # Utilize that bool->int conversion is true=1 and false=0
            a = (UB >= torch.abs(LB)).int()
        else:
            a = z
...
def bisection(l: torch.Tensor, u: torch.Tensor, f: TensorFunction, num_iter: int = 20) -> Tuple[torch.Tensor, torch.Tensor]:
    midpoint = (l + u) / 2
